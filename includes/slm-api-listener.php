...
class SLM_API_Listener {
    ...
    function creation_api_listener() {
        if (isset($_REQUEST['slm_action']) && trim($_REQUEST['slm_action']) == 'slm_create_new') {
		   ...
		   $fields['lic_status'] = (isset($_REQUEST['lic_status']))? strip_tags($_REQUEST['lic_status']): 'pending';
			...
			$fields['paypal_email'] = strip_tags($_REQUEST['paypal_email']);
			$fields['u_access_code'] = strip_tags($_REQUEST['u_access_code']);
			$fields['nanacast_id'] = strip_tags($_REQUEST['nanacast_id']);
    ...
    function check_api_listener() {
        if (isset($_REQUEST['slm_action']) && trim($_REQUEST['slm_action']) == 'slm_check') {
			...
			$registered_domain = trim(strip_tags($_REQUEST['registered_domain']));
            $sql_prep3 = $wpdb->prepare("SELECT * FROM $reg_table WHERE lic_key=%s AND registered_domain=%s", $key, $registered_domain);
            $reg_domains2 = $wpdb->get_results($sql_prep3, OBJECT);
			//echo '<pre>';
			//print_r($reg_domains2);
			$domainstatus = 'active';
			if(empty($reg_domains2)){
