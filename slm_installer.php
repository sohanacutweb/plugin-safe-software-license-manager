...
$lic_mail_table = SLM_TBL_LIC_MAIL_CONTENT;
$charset_collate = '';
...
$lk_tbl_sql = "CREATE TABLE " . $lic_key_table . " (
	  ...
	  u_access_code varchar(64) NOT NULL default '',
	  nanacast_id varchar(64) NOT NULL default '',
...
$mcont_tbl_sql = "CREATE TABLE " .$lic_mail_table. " (
      id INT NOT NULL AUTO_INCREMENT ,
...
dbDelta($mcont_tbl_sql);

$Maildata = $wpdb->get_results("SELECT * FROM $lic_mail_table ", ARRAY_A);
	if(empty($Maildata)){
		$wpdb->insert( 
			$lic_mail_table, 
			array( 
				'subject' => 'Your order is complete - download your files', 
				'body_content' => 'Hello [name], Your recent order on NanaCast has been completed. Your order details are shown below for your reference ', 
				'from_mail' => 'info@acutweb.com', 
			) 
		);
		$wpdb->insert( 
			$lic_mail_table, 
...
/*$options2 = array(
    'nanacast_outgoing_url' => 'nanacast-listener',
    
);
add_option('slm_plugin_nanacast_url', $options2);
*/
